defmodule LedgrWeb.Domains.CasaTame.IncomeController do
  ...
  def new(conn, _params) do
    changeset =
      Income.change_income_entry(%IncomeEntry{
  ...
  def create(conn, %{"income_entry" => attrs}) do
    ...
    case Income.create_income_entry_with_journal(attrs) do
      {:ok, entry} ->
        conn
        |> put_flash(:info, "Income recorded.")
        |> redirect(to: dp(conn, "/income/#{entry.id}"))
  ...
  def edit(conn, %{"id" => id}) do
    ...
    render(
      conn,
      :edit,
      [entry: entry, changeset: changeset, action: dp(conn, "/income/#{entry.id}")] ++
        form_assigns()
    )
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    case Income.delete_income_entry(entry) do
      {:ok, _} ->
        conn |> put_flash(:info, "Income deleted.") |> redirect(to: dp(conn, "/income"))
        ...
        from a in Account,
          where:
            (a.code >= "1000" and a.code <= "1019") or
              (a.code >= "1100" and a.code <= "1119"),
          order_by: [asc: a.code]
      )

    Enum.flat_map(@deposit_ranges, fn {_group_label, from_code, to_code, currency} ->
      group_accounts =
        Enum.filter(accounts, fn a -> a.code >= from_code and a.code <= to_code end)
