defmodule LedgrWeb.Domains.VolumeStudio.SubscriptionController do
  ...
  def index(conn, params) do
    ...
    db_plan_type =
      if current_plan_type in ["membership", "package", "promo", "extra"],
        do: current_plan_type,
        else: nil
  ...
  def cancel(conn, %{"id" => id} = params) do
    ...
    with :ok <- validate_refund(refund_cents, summary.total_paid),
      ...
      msg =
        if refund_cents > 0,
          do: "Subscription cancelled and refund recorded.",
      ...
      {:error, :invalid_refund} ->
        ...
        |> put_flash(
          :error,
          "Refund amount exceeds total paid ($#{:erlang.float_to_binary(summary.total_paid / 100, [{:decimals, 2}])} MXN)."
        )
  ...
  defp maybe_record_refund(sub, refund_cents, account_id, note) do
    case VolumeStudioAccounting.record_subscription_refund(
           sub,
           refund_cents,
           account_id,
           today_mx(),
