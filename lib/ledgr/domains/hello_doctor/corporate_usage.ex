defmodule Ledgr.Domains.HelloDoctor.CorporateUsage do
  ...
  *costs* in doctor payouts.
  ...
  Returns:
      ...
      %{
        ...
        months: [%{month: "2026-06", label: "Jun 2026", consults: 7,
                   cost: 700.0, cost_per_employee: 58.33}, ...] # newest first
      }
  """
  def summary(account_id, member_count) when is_binary(account_id) do
    rows = monthly_rows(account_id, member_count)
  ...
  def summary(_account_id, _member_count) do
    %{
      member_count: 0,
      cost_per_consult: cost_per_consult(),
  ...
  defp monthly_rows(account_id, member_count) do
    sql = """
    SELECT
      to_char(
